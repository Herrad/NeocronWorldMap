...
namespace NeocronWorldMap.Web.Services
{
    public class OutpostService : IRetrieveOutpostInformation
    {
        ...
        {
            var name = GetOutpostNameForCoordinates(coordinates);
            return new Outpost(name);
        }

        private string GetOutpostNameForCoordinates(Coordinates coordinates)
        {
            return _outpostLocations.NamesAt[coordinates];
        }
    }
